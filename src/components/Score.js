...
export function calculateLogScore(score) {
  const logScore =
    ((score - min) / (max - min)) * (Math.log10(max) - Math.log10(min)) +
    Math.log10(min);
...
export default function Score({ myColor, setStep, setMyColor, user }) {
  ...
  const [r, g, b] = cmykRgb([
    todaysColor.cyan / 100,
    todaysColor.magenta / 100,
    todaysColor.yellow / 100,
    todaysColor.black / 100,
  ]);
  ...
  if (!storedColor) {
    ...
    localStorage.setItem(
      `${new Date().toLocaleDateString()}_color`,
      `rgba(${myR},${myG},${myB},1)`,
    );
    saveScore();
  }

  const gradient = storedColor
    ? `linear-gradient(to right, ${storedColor} 50%, rgba(${r}, ${g}, ${b}, 1) 50%)`
    : `linear-gradient(to right, rgba(${myR},${myG},${myB},1) 50%, rgba(${r}, ${g}, ${b}, 1) 50%)`;
  ...
  return (
    ...
    >
      <div>
        Score:{" "}
        {storedScore
          ? calculateLogScore(storedScore)
          : calculateLogScore(score)}
      </div>
      <Button onClick={() => setStep(4)}>Previous scores</Button>
      <Button
        style={{
          position: "absolute",
          bottom: 0,
